class FakePaymentGroup
  ...
  constructor: ->
    ...
    # Deshabilitar 'FakeMode' al clickear en 'Volver a Pago Real'
    ...
    $('[data-behavior~=displayIncome]').on 'click', =>
      $("input[name='register_ids[]']").prop('checked', false)
      @resetSelectedServices()

    # Resetear cuenta bancaria al hacer click en 'Devolución de reservas'
  ...
  getServiceRegisters: =>
    ...
    $.ajax
      url: "/service_registers.js?register_filter[operator_id]=#{id}&register_filter[credits_tab]=true"
      cache: false
      dataType: 'html'
      success: (data) =>
  ...
  sumSelectedServices: =>
    ...
    @validateTotal(Math.abs(ars), Math.abs(usd))

  validateTotal: (ars, usd) ->
    submit     = $('#js-btn-payment')
    data       = $('#js-total_debt')
    total      = Math.abs(parseFloat(data.data('total')))
  ...
  displayHelp: ->
    $('#modal').find('.modal-content').html("<div class='modal-header'><h4 class='modal-title'><i class='fa fa-question-circle text-primary'></i> Ayuda</h4></div><div class='modal-body'><div class='row'><div class='col-md-12'><p>La funcionalidad de <b>pagos ficticios con compensación por devolución de reservas</b> utiliza cuentas de ajuste (<b>AJUSTE</b> o <b>AJUSTE DÓLARES</b>) para pagar dinero de débitos y devolver con la misma cuenta créditos de otro operador.</p><p>La carga <b>asienta un pago y una devolución en Aptour</b> con la cuenta de ajuste seleccionada.</p><p>La única restricción de la funcionalidad es que <b>no puede utilizarse la cuenta AJUSTE DÓLARES para pagar o compensar servicios nominados en pesos</b>, ya sea débitos de pago o créditos de compensación.</p></div></div></div>")
    $('#modal').modal()


$(document).on 'ready turbolinks:load', ->
  if $('.refund_payment_groups').length == 0 && ($('.international_payment_groups').length > 0 || $('.national_payment_groups').length > 0)
    new FakePaymentGroup()
